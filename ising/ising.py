...
class grid:
    ...
    def ising(self,T=1):
        ...
        self.grid[randx][randy]=self.calcNewVal(randx, randy,T=1)
    ...
    def similtaneousIsing(self,T=1):
        ...
        for x in xrange:
            for y in yrange:
                newgrid[x][y]=self.calcNewVal(x, y,T=1)
    ...
    def mp4ising(self,N,T=1,f=25, run=1, maxRun=1):
        self.plot(True,T)
        
        for i in range(N):
            self.ising(T)
            self.plot(True,T)
            print(f'{(i+1)/N*100}% done run {run} of {maxRun}')
            
        files=sorted(glob.glob(f'gridplot_t*_T{T}_{self.getx()}x{self.gety()}grid.png'), key=os.path.getmtime)
        
        with iio.get_writer(f'ising_T{T}_{f}fps_{N}N_{self.getx()}x{self.gety()}grid.mp4', fps=f) as writer:
            for file in files:
                image=iio.imread(file)
                writer.append_data(image)
                
    def mp4wTs(self, N, Tlist,f=25):
        maxRun=len(Tlist)
        run = 1
        
        for T in Tlist:
            self.mp4ising(N,T,f,run,maxRun)
            self.reset()
            run +=1
